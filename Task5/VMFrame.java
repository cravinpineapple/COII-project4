...
class VMFrame extends JFrame {
	...
	private int[] statusTLBRowFIFO = new int[4];
	private int statusRowLRU = 0;		//INCREMENTED EACH TIME A TLB ROW IS ACCESSED
	private int statusRowFIFO = 0;		// INCREMENTED AND ASSIGNED EACH TIME NEW TLB ADDED
	...
	public VMFrame(){
		...
		for (int i = 0; i < 4; i++){
			...
			statusTLBRowFIFO[i] = 0;
		}
		
		statusMemFrameFIFO = 0;
		statusRowLRU = 0;
		statusRowFIFO = 0;
		...
		while (tempState <= moveStatus){
			...
			case 3: //SEARCH THE TLB FOR THE VIRTUAL PAGE NUMBER AND HIGHLIGHT THE RESULT, IF FOUND
				...
				if (tlbEntry >= 0){
					...
					// **Removed because we don't need to change FIFO information on hit
					/*
					statusRowFIFO++;
					statusTLBRowFIFO[tlbEntry] = statusRowFIFO;		
					*/								
					...
					if (pageTableEntry >= 0){//THERE WAS A PAGE HIT
						...
						if (!(emptyTLBRow == -1)){		//IF THERE IS AN EMPTY TLB ROW				
							...
							//** we DO want this because, we want to increment on insertion
							statusRowFIFO++;
							statusTLBRowFIFO[emptyTLBRow] = statusRowFIFO;	
							
							//GLOBAL VARIABLE THAT HOLDS THE CURRENT ROW IN THE TLB
							tlbEntry = emptyTLBRow;
						}
						else { //GET THE FIFO TLB ROW AND REPLACE IT WITH THE NEW INFO
							tlbPanel.virtPageNum[getFIFORow()] = ""+intVirtPageDec;
							tlbPanel.physPageNum[getFIFORow()] = ""+pageTableEntry;
						
							//UPDATE THE FIFO STATUS OF THE TLB ROW
							statusRowFIFO++;
							statusTLBRowFIFO[getFIFORow()] = statusRowFIFO;
			...
			case 4: //HIGHLIGHT THE PHYSICAL MEMORY FRAME WHERE THE PAGE EXISTS OR WILL EXIST
				...
				if (boolTLBHit){
					...
					statusMemFrameFIFO++;
					statusFrameFIFO[Integer.parseInt(tlbPanel.physPageNum[tlbEntry], 10)] = statusMemFrameFIFO;
				...
				//CASE 2: THERE WAS A PAGE TABLE HIT
					...
					//UPDATE THE LRU STATUS OF THE MEMORY FRAME
					statusMemFrameFIFO++;
					statusFrameFIFO[pageTableEntry] = statusMemFrameFIFO;
				...
				//THIS FRAME MAY BE AN AVAILABLE EMPTY FRAME IN MEMORY, OR THE LEAST RECENTLY USED ONE
					...
					//ELSE DISPLAY THE LRU MEMORY FRAME WHICH WILL BE REPLACED
						...
						fifoMemFrame = getFIFOMemFrame();

						//HIGHLIGHT THE MEMORY FRAME IN YELLOW
						physMemPanel.boolFrames[fifoMemFrame] = true;
						
						//STORE THE MEMORY FRAME NUMBER IN A GLOBAL INT	
						frameNum = fifoMemFrame;
					}				
			...
			case 6: //BRING THE PAGE INTO MEMORY AND HIGHLIGHT THE FRAME WITH THE DATA IN IT
				...
				if (boolTLBHit || boolPageHit){
					...
					if (fifoMemFrame >= 0){
						String pageNum = physMemPanel.stringWords[fifoMemFrame][0].substring(1, 2);
					...
					statusMemFrameFIFO++;
					statusFrameFIFO[frameNum] = statusMemFrameFIFO;
	...
	*/
	...
	public int getLRUMemFrame(){
		...
		//COMPARE THE VALUES OF THE ARRAY statusFrameFIFO AND RETURN THE INDEX WITH THE MINIMUM VALUE
		for (int i = 1; i < 4; i++){
			if (statusFrameFIFO[minimum] > statusFrameFIFO[i])
				minimum = i;
		}
		return minimum;

	}//END FUNCTION getFIFOMemFrame
	
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	//FUNCTION TO RETURN THE INDEX OF THE INDEX THAT WAS FIRST IN
	
		public int getFIFOMemFrame() {
			int firstIn = 0;
			
			for (int i = 1; i < 4; i++) {
				if (statusTLBRowFIFO[firstIn] > statusTLBRowFIFO[i])
					firstIn = i;
			}

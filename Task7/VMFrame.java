...
class VMFrame extends JFrame {
	...
	private int lfuMemFrame = -1;
	private int frameNum = -1;
	private boolean[] statusFrameEmpty = new boolean[4];
	private int[] statusFrameLFU = new int[4];
	private int statusMemFrameLFU = 0;		//INCREMENTED EACH TIME A FRAME IS ACCESSED
	...
	private int[] statusTLBRowLFU = new int[4];
	private int statusRowLFU = 0;		//INCREMENTED EACH TIME A TLB ROW IS ACCESSED
		...
		while (tempState <= moveStatus){
			...
			case 3: //SEARCH THE TLB FOR THE VIRTUAL PAGE NUMBER AND HIGHLIGHT THE RESULT, IF FOUND
				...
				if (tlbEntry >= 0){
					...
					// usage increases by 1
					statusTLBRowLFU[tlbEntry]++;										
			...
			case 4: //HIGHLIGHT THE PHYSICAL MEMORY FRAME WHERE THE PAGE EXISTS OR WILL EXIST
				...
				if (boolTLBHit){
					...
					//statusMemFrameLRU++;
					statusFrameLFU[Integer.parseInt(tlbPanel.physPageNum[tlbEntry], 10)]++;
				...
				//THIS FRAME MAY BE AN AVAILABLE EMPTY FRAME IN MEMORY, OR THE LEAST RECENTLY USED ONE
					...
					//ELSE DISPLAY THE LRU MEMORY FRAME WHICH WILL BE REPLACED
						...
						lfuMemFrame = getLRUMemFrame();

						//HIGHLIGHT THE MEMORY FRAME IN YELLOW
						physMemPanel.boolFrames[lfuMemFrame] = true;
						
						//STORE THE MEMORY FRAME NUMBER IN A GLOBAL INT	
						frameNum = lfuMemFrame;
					}				
			...
			case 6: //BRING THE PAGE INTO MEMORY AND HIGHLIGHT THE FRAME WITH THE DATA IN IT
				...
				if (boolTLBHit || boolPageHit){
					...
					if (lfuMemFrame >= 0){
						String pageNum = physMemPanel.stringWords[lfuMemFrame][0].substring(1, 2);
					...
					if (!(emptyTLBRow == -1)){ //AN EMPTY TLB ROW IS AVAILABLE	
						...
						//statusRowLRU++;
						statusTLBRowLFU[getLFURow()] = 1;// = statusRowLRU;
	...
	*/
	...
	public int getLRUMemFrame(){
		...
		for (int i = 1; i < 4; i++){
			if (statusTLBRowLFU[lfuIndex] >= statusTLBRowLFU[i])
				lfuIndex = i;
		}
		return lfuIndex;
